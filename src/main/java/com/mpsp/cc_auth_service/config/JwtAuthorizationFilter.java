...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    @Autowired
    private transient JwtTokenProvider tokenProvider;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        //checking if token is present and is of bearer type. Also checking if the request is for refreshing token
        if (GeneratorUtils.checkIfUrlEndsWith(request.getRequestURI(), skipAuthorizationUrls) || GeneratorUtils.checkIfUrlsContainUri(request.getRequestURI(), skipTokenCheckPaths)) {
        ...
        if (StringUtils.isBlank(token) || !token.startsWith(AppConstants.BEARER)) {
            log.error("Authorization token is blank or of not of bearer type on {}", request.getRequestURI());
        ...
        try {
            ...
            if (tokenProvider.verifyToken(token, userId, false)) {
                UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(tokenProvider.getSubject(token), null, null);
                SecurityContextHolder.getContext().setAuthentication(authentication);
                filterChain.doFilter(request, response);
            } else {
                log.error("Cannot Authorize {} to access {}", userId, request.getRequestURI());
                response.getWriter().write(new ObjectMapper().writeValueAsString(new ErrorResponse("Unauthorized", "Invalid Token")));
                response.setStatus(HttpStatus.UNAUTHORIZED.value());
                response.setContentType(MediaType.APPLICATION_JSON_VALUE);
            }
        } catch (Exception e) {
